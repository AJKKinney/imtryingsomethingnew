...
export const DECISIONS: readonly Decision[] = Object.freeze([
  ...
  { topic: 'the prototype carries its own tick gate', owner: 'commit 10', system: 'ui',
    decision: 'The workbench advances heat on whole DT steps against an interval of TICK_MS / 0.2, because §9 runs the board at 20% time and §142.4 makes a time-scale a TICK GATE rather than a `dt` multiplier. A prototype with no world still obeys the rule the world obeys, so the heat curve a playtester sees at the gate is the curve the loop will produce when it wires the board — and nothing here has to be un-learned later.' },
